 ...
 * - Social media links
 * - Mobile menu toggle with sliding menu
 */
export default function Header() {
  ...
  return (
    <>
      <header className="w-full py-8 px-8 md:px-12 lg:px-16 flex items-center justify-between relative z-50">
        {/* Logo and Brand */}
        <div className="flex items-center gap-4">
          <div className="flex items-center justify-center">
            <Image src="/logo.png" alt="CreatorLoop Logo" width={48} height={48} />
          </div>
          <button 
            onClick={() => scrollToSection('hero')}
            className="text-2xl font-bold font-red-hat-display hover:opacity-80 transition-opacity"
          >
        ...
        {/* Desktop Navigation */}
        <nav className="hidden lg:flex items-center gap-8 font-red-hat-text">
          {navigationLinks.map((link) => (
            <button
              key={link.id}
              onClick={() => scrollToSection(link.id)}
              className="text-lg text-slate-700 hover:text-purple-600 transition-colors font-medium hover:cursor-pointer"
            >
              {link.name}
            </button>
          ))}
        ...
        <div className="flex items-center gap-6">
          ...
          <button 
            className="lg:hidden text-slate-700 z-50 relative"
          ...
          >
            <svg 
              ...
              className={`transform transition-transform duration-300 ${isMobileMenuOpen ? 'rotate-45' : ''}`}
            >
              {isMobileMenuOpen ? (
                <>
      ...
      <div 
        className={`fixed top-0 right-0 h-full w-80 bg-white/95 backdrop-blur-xl border-l border-white/20 z-40 lg:hidden transform transition-transform duration-300 ${
      ...
      >
        <div className="flex flex-col pt-24 px-8">
          <nav className="flex flex-col gap-6 mb-8">
            {navigationLinks.map((link) => (
              <button
                key={link.id}
                onClick={() => scrollToSection(link.id)}
                className="text-xl text-slate-700 hover:text-purple-600 transition-colors font-medium text-left py-2 font-red-hat-text"
              >
                {link.name}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-4 pt-6 border-t border-slate-200">
            <h3 className="text-lg font-medium text-slate-700 mb-2 font-red-hat-text">Follow Us</h3>
            <div className="flex items-center gap-4">
              {socialLinks.map((social) => (
                social.href ? (
                  <Link
