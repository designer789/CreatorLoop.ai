 ...
 * - Logo and brand name
 * - Navigation links to sections with smooth scroll
 * - Social media links
...
export default function Footer() {
  ...
  const socialLinks = [
    {
      name: 'Twitter',
      href: 'https://x.com/CreatorLoop_ai',
      icon: (
    ...
    {
      name: 'Gitbook',
      href: 'https://docs.creatorloopai.xyz',
      icon: (
  ...
  return (
    <footer className="w-full bg-gradient-to-br from-slate-50 to-white border-t border-slate-200">
      <div className="w-full mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 py-12 sm:py-16">
        {/* Main Footer Content */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10 lg:gap-12 mb-8 sm:mb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
              <div className="flex items-center gap-3">
                <div className="flex items-center justify-center">
                  <Image src="/logo.png" alt="CreatorLoop Logo" width={40} height={40} className="sm:w-12 sm:h-12" />
                </div>
                <button 
                  onClick={() => scrollToSection('hero')}
                  className="text-xl sm:text-2xl font-bold font-red-hat-display hover:opacity-80 transition-opacity"
                >
                  CreatorLoop<span className="text-purple-600">.ai</span>
                </button>
              </div>
            </div>
            <p className="text-slate-600 font-red-hat-text text-base sm:text-lg leading-relaxed max-w-full sm:max-w-md">
              Empowering creators with AI-driven tools to build, monetize, and scale their digital presence through innovative blockchain technology.
            </p>
          </div>

          {/* Navigation Links */}
          <div className="order-2 sm:order-none">
            <h3 className="text-base sm:text-lg font-semibold text-slate-800 mb-4 sm:mb-6 font-red-hat-display">Navigation</h3>
            <nav className="grid grid-cols-2 sm:flex sm:flex-col gap-3 sm:gap-4">
              {navigationLinks.map((link) => (
                <button
                  key={link.id}
                  onClick={() => scrollToSection(link.id)}
                  className="text-sm sm:text-base text-slate-600 hover:text-purple-600 transition-colors font-medium text-left font-red-hat-text hover:cursor-pointer"
                >
                  {link.name}
                </button>
              ))}
          ...
          <div className="order-3 sm:order-none">
            <h3 className="text-base sm:text-lg font-semibold text-slate-800 mb-4 sm:mb-6 font-red-hat-display">Follow Us</h3>
              ...
              {socialLinks.map((social) => (
                social.href ? (
                  <Link
                    ...
                    className="flex items-center gap-3 text-sm sm:text-base text-slate-600 hover:text-purple-600 transition-colors font-red-hat-text"
                  >
                    <span className="p-1 sm:p-2">{social.icon}</span>
                    {social.name}
                  </Link>
                ) : (
                  <div
                    key={social.name}
                    className="flex items-center gap-3 text-sm sm:text-base text-slate-400 font-red-hat-text"
                  >
                    <span className="p-1 sm:p-2">{social.icon}</span>
                    {social.name} <span className="text-xs">(Coming Soon)</span>
                  </div>
                )
              ))}
            </div>
        ...
        <div className="border-t border-slate-200 pt-6 sm:pt-8">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-6">
            {/* Copyright */}
            <p className="text-sm sm:text-base text-slate-500 font-red-hat-text text-center sm:text-left order-2 sm:order-1">
              © {currentYear} CreatorLoop.ai. All rights reserved.
            </p>

            {/* Additional Social Icons for Desktop */}
